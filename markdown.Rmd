...
perf_cnt_diff_dur <- summary_performance %>%
        ...
        group_by(interval_dur = cut(perf_diff_dur, breaks = intervals, right=FALSE)) %>%
        count() %>%
        spread(interval_dur, n)

perf_cnt_diff_af <- summary_performance %>%
        select(perf_diff_af) %>%
        group_by(interval_af = cut(perf_diff_af, breaks = intervals, right=FALSE)) %>%
        count() %>%
        spread(interval_af, n)


inferential_summary_perf <- bind_rows(perf_cnt_bef, perf_cnt_dur, perf_cnt_af, perf_cnt_diff_dur, perf_cnt_diff_af) %>%
        mutate(index = c("Before COVID", "During COVID", "After COVID", "Difference during COVID", "Difference after COVID"))
